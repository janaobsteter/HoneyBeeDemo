...
for chromosome in range(1, 17):
    print(chromosome)
    drone_ts = tskit.load("Chr" + str(chromosome) + "_processed.trees")
    ...
    fstLineageDFChrPer.to_csv("FstLineageChrPer.csv", index = False)
    ...
    fstSubspecieDFChrPer.to_csv("FstSubspecieChrPer.csv", index = False)
